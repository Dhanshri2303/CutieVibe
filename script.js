...
function fetchProducts() {
    db.ref('products/').on('value', (snapshot) => {
        ...
        allProducts = [];
        if (data) {
            Object.keys(data).forEach(key => allProducts.push(data[key]));
            allProducts.reverse();
...
function displayProducts(products) {
    ...
    products.forEach((item) => {
        ...
        card.innerHTML = `
            ...
            <button class="wishlist-btn ${isWish ? 'active' : ''}" onclick="toggleWishlist('${item.title}', this)">${isWish ? '💖' : '♡'}</button>
            ...
            <button class="buy-btn" style="background:#333;" onclick="addToCart('${item.title}', '${item.price}')">Add To Bag 🛍️</button>
        `;
...
// Updated fetchProducts to read directly from local products.json
async function fetchProducts() {
    try {
        const response = await fetch('products.json');
        const rawData = await response.json();
        
        // Map local products.json structure to match UI expectations
        allProducts = rawData.map(item => ({
            title: item.name ? (item.brand ? `${item.brand} - ${item.name}` : item.name) : 'Cute Product',
            price: `₹${item.sellPrice || item.mrp || 0}`,
            image: item.image && item.image !== "https://via.placeholder.com/200" 
                    ? item.image 
                    : 'https://images.unsplash.com/photo-1522337360788-8b13dee7a37e?w=500', // aesthetic fallback image
            category: item.category || 'all',
            badge: item.discount ? `${item.discount} OFF` : 'Trending 🔥',
            link: '#'
        }));

        // Render first 60 products for optimal UI performance
...
// Auto-fetch products on page load
